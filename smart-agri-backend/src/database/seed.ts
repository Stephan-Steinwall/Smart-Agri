...
async function bootstrap() {
    ...
    if (!supabaseUrl || !supabaseKey) {
        console.error('❌ SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY missing. Cannot seed.');
        process.exit(1);
    }
    const supabase = createClient(supabaseUrl, supabaseKey);

    console.log('🧹 Clearing old data (requires RLS bypass or TRUNCATE in SQL)...');
    ...
    if (latestRecord) {
        ...
        const { error } = await supabase.from('latest_soil_reading').upsert(latestRecord, { onConflict: 'device_id' });
        if (error) {
    ...
    const chatHistory = [
        ...
        },
        {
            session_id: sessionId,
            role: 'assistant',
            content: 'Based on the latest data, your soil moisture is optimal, but nitrogen levels are dropping slightly. Monitor it over the next 48 hours.'
        }
    ];

    const { error: chatError } = await supabase.from('ai_chat_history').insert(chatHistory);
    if (chatError) {
