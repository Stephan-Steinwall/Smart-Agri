...
export class AiService {
    ...
    private calculateCropScore(reading: any, profile: any): number {
        ...
        if (reading.moisture != null) {
            scores.push(this.clampScore(100 - Math.abs(reading.moisture - profile.idealMoisture) * 1.2));
        }
        if (reading.ph != null) {
            scores.push(this.clampScore(100 - Math.abs(reading.ph - profile.idealPh) * 20));
        }
        if (reading.temperature != null) {
            scores.push(this.clampScore(100 - Math.abs(reading.temperature - profile.idealTemperature) * 2));
        }
        if (reading.nitrogen != null) {
            scores.push(this.clampScore(100 - Math.abs(reading.nitrogen - profile.idealNitrogen) * 1.2));
        }
        if (reading.phosphorus != null) {
            scores.push(this.clampScore(100 - Math.abs(reading.phosphorus - profile.idealPhosphorus) * 2));
        }
        if (reading.potassium != null) {
            scores.push(this.clampScore(100 - Math.abs(reading.potassium - profile.idealPotassium) * 1.4));
        }
    ...
    private buildCropPrompt(reading: any): string {
        ...
        const profileLines = this.cropProfiles
            .map((profile) => `- ${profile.name}: moisture ${profile.idealMoisture}%, pH ${profile.idealPh}, temperature ${profile.idealTemperature}°C, nitrogen ${profile.idealNitrogen} ppm, phosphorus ${profile.idealPhosphorus} ppm, potassium ${profile.idealPotassium} ppm.`)
            .join('\n');
...
Do not add any extra commentary or fields.`;
    ...
    private buildPrePlantRecommendations(reading: any, profile: any): string[] {
        ...
        if (reading.moisture != null) {
            ...
            if (Math.abs(diff) > 5) {
                ...
                recommendations.push(`Keep soil moisture near ${profile.idealMoisture}% before planting.`);
            }
        }
        ...
        if (reading.nitrogen != null) {
            ...
            if (Math.abs(diff) > 10) {
                ...
                recommendations.push(`Maintain nitrogen near ${profile.idealNitrogen} ppm.`);
            }
        }
        ...
        if (reading.phosphorus != null) {
            ...
            if (Math.abs(diff) > 10) {
                ...
                recommendations.push(`Keep phosphorus near ${profile.idealPhosphorus} ppm.`);
            }
        }
        ...
        if (reading.potassium != null) {
            ...
            if (Math.abs(diff) > 10) {
                ...
                recommendations.push(`Keep potassium near ${profile.idealPotassium} ppm.`);
            }
        }

        if (recommendations.length === 0) {
            recommendations.push(`Review soil moisture, pH, and nutrient balance before planting ${profile.name}.`);
        }
    ...
    private buildEvaluateCropFallback(reading: any, cropName: string) {
        ...
        const profile = this.cropProfiles.find((profile) => profile.name.toLowerCase() === normalizedName);
        ...
        if (profile) {
            ...
            if (reading.moisture != null) {
                const diff = reading.moisture - profile.idealMoisture;
                reasons.push(`Moisture is ${reading.moisture.toFixed(0)}%, ideal for ${profile.name} is ${profile.idealMoisture}%.`);
                actions.push(diff > 5
            ...
            if (reading.ph != null) {
                const diff = reading.ph - profile.idealPh;
                reasons.push(`pH is ${reading.ph.toFixed(1)}, ideal for ${profile.name} is ${profile.idealPh}.`);
                actions.push(diff > 0.3
    ...
    async evaluateCrop(deviceId: string, cropName: string) {
        ...
        try {
            const response = await this.openai.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [
                    { role: 'system', content: 'You are a concise agronomist helping farmers decide whether to plant a specific crop based on soil sensor data.' },
            ...
            if (content) {
                ...
                if (parsed?.cropName && typeof parsed?.canPlantNow === 'boolean' && Array.isArray(parsed?.actions)) {
                    return parsed;
                }
            }
        } catch (error) {
            this.logger.warn(`AI crop evaluation failed, using fallback analysis: ${error?.message ?? error}`);
        }
    ...
    async suggestCrop(deviceId: string) {
        ...
        try {
            const response = await this.openai.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [
                    { role: 'system', content: 'You are a concise agronomist helping farmers choose the best crop based on soil data.' },
                    { role: 'user', content: prompt },
                ],
                temperature: 0.2,
            });
            ...
            if (content) {
                const parsed = JSON.parse(content);
                if (parsed?.recommendedCrop && Array.isArray(parsed?.alternatives) && Array.isArray(parsed?.prePlantRecommendations)) {
                    return parsed;
                }
            }
        } catch (error) {
            this.logger.warn(`AI crop suggestion failed, falling back to heuristic scoring: ${error?.message ?? error}`);
        }
        ...
        const rankedCrops = this.cropProfiles
            .map((profile) => {
                ...
                if (reading.moisture != null) {
                    reasons.push(`Moisture ${reading.moisture.toFixed(0)}% aligns with ${profile.name.toLowerCase()} growing needs.`);
                }
                if (reading.ph != null) {
                    reasons.push(`Soil pH ${reading.ph.toFixed(1)} is compatible with ${profile.name.toLowerCase()} cultivation.`);
                }

                return { crop: profile.name, score, reasons, description: profile.description };
            })
            .sort((a, b) => b.score - a.score);

        const [recommendedCrop, ...alternatives] = rankedCrops;
        const selectedProfile = this.cropProfiles.find((profile) => profile.name === recommendedCrop.crop);

        return {
            recommendedCrop,
            alternatives: alternatives.slice(0, 2),
            summary: `Based on the latest readings, ${recommendedCrop.crop} is the strongest match for this field.`,
            prePlantRecommendations: selectedProfile ? this.buildPrePlantRecommendations(reading, selectedProfile) : [],
        };
    }
